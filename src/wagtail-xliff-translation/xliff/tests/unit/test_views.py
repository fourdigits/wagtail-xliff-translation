...
from zg.django.xliff.serializers.wagtail_xliff import WagtailXliffSerializer
from zg.django.xliff.serializers.xliff_wagtail import XliffWagtailDeserializer

pytestmark = pytest.mark.django_db


def test_download_view_post_invalid(admin_client, zg_page_factory):
...
def test_download_view_with_descendants(admin_client, zg_page_factory):
    page = zg_page_factory()
    zg_page_factory(parent=page)
...
def test_download_view_with_included_subtree(
    admin_client, zg_page_factory, language_factory
):
...
def test_download_view_without_admin_rights(client, zg_page_factory):
    page = zg_page_factory()
...
def test_download_view_serialization_error(
    admin_client, zg_page_factory, language_factory, mocker
):
    page = zg_page_factory()
    german = language_factory(code="de")
...
def test_upload_view_post_deserialization_error(admin_client, zg_page_factory, mocker):
    page = zg_page_factory()
...
def test_upload_view_post_invalid(admin_client, zg_page_factory):
    page = zg_page_factory()
    resp = admin_client.post(
        reverse("xliff:upload", kwargs={"page_id": page.pk}),
        {
